...
class FeatureExract(nn.Module):
    ...
    ##
    def forward(self,images,gt_ID):
        features = self.resnet(images)
        features = features.reshape(features.size(0),-1)
        
        #features = self.linear(features)
        
        features_norm = torch.norm(features,p=2,dim=1).cuda()
        #print(type(features_norm))
        features_norm = torch.reshape(features_norm,(-1,features_norm.size()[0])).cuda()
        
        features_norm = features_norm.transpose(1,0).cuda()
        features = features/features_norm
        #
        weight = self.linear.weight      
        weight_norm = torch.norm(weight,p=2,dim=1).cuda()
        weight_norm = torch.reshape(weight_norm,(-1,weight_norm.size()[0]))
        #print(weight_norm.size())
        weight_norm = weight_norm.transpose(1,0)
        weight = weight/weight_norm 
      
        features = torch.mm(features,weight.transpose(1,0)).cuda()

        row = torch.from_numpy(np.array(range(0,gt_ID.size(0)))).long()
        column = torch.reshape(gt_ID,(gt_ID.size(0),)).long()
        original_target_logit = torch.reshape(features[row,column],(-1,1))
        
        #print('ori',original_target_logit)
        #print(gt_ID)
        theta = torch.acos(original_target_logit)   
        mariginal_target_logit = torch.cos(theta+0.9) 
        one_hot = torch.randn([1,5])
        one_hot = one_hot==one_hot[0,gt_ID]
        one_hot = one_hot.cuda().float()
        features = features+one_hot*(mariginal_target_logit-original_target_logit)
